...
test("retrieval context includes entity cards for named-person queries", async () => {
  ...
  const context = await buildRetrievalContext(repository, conversation.id, "Person1 是谁？", {});
  ...
  assert.equal(context.entityCards[0].name, "Person1");
  ...
  assert.match(promptContext, /Person1 是用户近期重要的亲密关系对象/);
...
test("time-sensitive retrieval includes deterministic timeline events", async () => {
  ...
  const conversation = repository.createConversation("main");
  const userMessage = repository.createMessage(conversation.id, "user", "Person1 去了 Truffles。");
  repository.createMemoryEvent({
    conversationId: conversation.id,
    sourceMessageId: userMessage.id,
    summary: "用户去 Truffles 找 Person1，但 Person1 不在",
    score: 0.8,
  ...
  assert.match(promptContext, /\[2026-05-05\] 用户去 Truffles 找 Person1/);
...
test("entity time queries filter timeline events to that entity", async () => {
  ...
  const conversation = repository.createConversation("main");
  const userMessage = repository.createMessage(conversation.id, "user", "Person1 和 capstone 都有事。");
  repository.createMemoryEvent({
    conversationId: conversation.id,
    sourceMessageId: userMessage.id,
    summary: "用户描述 Person1：Person1 被公司临时召回",
    score: 0.8,
    occurredAt: "2026-05-05",
  });
  repository.createMemoryEvent({
  ...
  const context = await buildRetrievalContext(repository, conversation.id, "Person1 昨天发生了什么？", {});

  assert.equal(context.timelineEvents.length, 1);
  assert.match(context.timelineEvents[0].summary, /Person1/);
});

test("retrieval router separates relationship, health, correction, and open thinking routes", () => {
  assert.equal(classifyRetrievalRoute("我对PartnerA有分离焦虑吗？").type, "relationship_reasoning");
  assert.equal(classifyRetrievalRoute("Person1 发烧是不是 HIV？").type, "health_risk");
...
test("value execution check adds route-specific guardrails for health and relationship turns", () => {
  const healthPrompt = buildPromptContext({
    ...
    entityCards: [],
    timelineEvents: [],
    retrievalRoute: classifyRetrievalRoute("Person1 发烧是不是 HIV？"),
  });
  ...
  const relationshipPrompt = buildPromptContext({
    ...
    entityCards: [],
    timelineEvents: [],
    retrievalRoute: classifyRetrievalRoute("我对PartnerA有分离焦虑吗？"),
  });
